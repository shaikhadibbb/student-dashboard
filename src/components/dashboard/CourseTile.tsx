...
export function CourseTile({ course }: { course: Course }) {
  ...
  return (
    <motion.article 
      className="relative overflow-hidden rounded-2xl bg-card-gradient border border-border-subtle p-5 md:p-6 group cursor-pointer h-full flex flex-col justify-between"
      whileHover={{ 
        scale: 1.02,
    ...
    >
      ...
      <div className="absolute inset-0 rounded-2xl border border-accent-glow opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
